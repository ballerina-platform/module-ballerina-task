...
public class SchedulerActions {
    ...
    public static Object init(Environment env, BObject taskScheduler) {
        ...
        int threadCount = (int) taskScheduler.getIntValue(TaskConstants.THREAD_COUNT);
        int threadPriority = (int) taskScheduler.getIntValue(TaskConstants.THREAD_PRIORITY);
        int thresholdInMillis = (int) taskScheduler.getIntValue(TaskConstants.THRESHOLD_IN_MILLIS);
        try {
            StdSchedulerFactory stdSchedulerFactory = new StdSchedulerFactory(
                    createSchedulerProperties(threadCount, threadPriority, thresholdInMillis));
    ...
    public static Object scheduleJob(BObject taskScheduler, BObject job, BMap<BString, Object> config) {
        ...
        try {
            jobScheduler.scheduleJob(jobDataMap, config, jobId);
        } catch (SchedulerException e) {
            return Utils.createSchedulerError(e.getMessage());
        }
    ...
    public static Properties createSchedulerProperties(int threadCount, int threadPriority, int thresholdInMillis) {
